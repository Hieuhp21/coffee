...
db.serialize(() => {
    ...
    const users = [
        { username: 'farmer1', password: '123', role: 'farmer', info: JSON.stringify({ name: 'Nguyễn Văn A', address: 'Đắk Lắk', area: '2.5 ha' }) },
        { username: 'processor1', password: '123', role: 'processor', info: JSON.stringify({ name: 'Nhà máy ABC', address: 'KCN XYZ', capacity: '1000 kg/ngày' }) },
        { username: 'packer1', password: '123', role: 'packer', info: JSON.stringify({ name: 'Xưởng XYZ', address: 'KCN ABC' }) },
        { username: 'transporter1', password: '123', role: 'transporter', info: JSON.stringify({ name: 'Công ty ABC', address: 'TP. Buôn Ma Thuột' }) },
        { username: 'retailer1', password: '123', role: 'retailer', info: JSON.stringify({ name: 'Cửa hàng XYZ', address: 'TP.HCM' }) },
    ...
    users.forEach(user => {
        bcrypt.hash(user.password, 10, (err, hash) => {
            if (!err) {
                db.run(`INSERT OR IGNORE INTO users (username, password, role, info) VALUES (?, ?, ?, ?)`, 
...
app.get('/dashboard', authenticateToken, (req, res) => {
    db.get(`SELECT info FROM users WHERE id = ?`, [req.user.id], (err, row) => {
        ...
        res.render('dashboard', { role: req.user.role, username: info.name || `User_${req.user.id}` });
    });
});
...
app.get('/farmer/history', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM history WHERE role = 'farmer'`, (err, rows) => {
        res.render('farmer/history', { username: `User_${req.user.id}`, history: rows });
    });
});

// Chế biến
app.get('/processor/info', authenticateToken, (req, res) => {
    if (req.user.role !== 'processor') return res.status(403).send('Không có quyền');
    db.get(`SELECT info FROM users WHERE id = ?`, [req.user.id], (err, row) => {
...
app.get('/processor/history', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM history WHERE role = 'processor'`, (err, rows) => {
        res.render('processor/history', { username: `User_${req.user.id}`, history: rows });
    });
});
...
app.get('/packer/history', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM history WHERE role = 'packer'`, (err, rows) => {
        res.render('packer/history', { username: `User_${req.user.id}`, history: rows });
    });
});

// Vận chuyển
app.get('/transporter/info', authenticateToken, (req, res) => {
    if (req.user.role !== 'transporter') return res.status(403).send('Không có quyền');
    db.get(`SELECT info FROM users WHERE id = ?`, [req.user.id], (err, row) => {
...
app.get('/transporter/history', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM history WHERE role = 'transporter'`, (err, rows) => {
        res.render('transporter/history', { username: `User_${req.user.id}`, history: rows });
    });
});

// Bán lẻ
app.get('/retailer/info', authenticateToken, (req, res) => {
    if (req.user.role !== 'retailer') return res.status(403).send('Không có quyền');
    db.get(`SELECT info FROM users WHERE id = ?`, [req.user.id], (err, row) => {
...
app.get('/retailer/history', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM history WHERE role = 'retailer'`, (err, rows) => {
        res.render('retailer/history', { username: `User_${req.user.id}`, history: rows });
    });
});
...
app.get('/admin/users', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM users`, (err, rows) => {
        res.render('admin/users', { username: `Admin_${req.user.id}`, users: rows });
    });
});
...
app.get('/admin/history', authenticateToken, (req, res) => {
    ...
    db.all(`SELECT * FROM history`, (err, rows) => {
        res.render('admin/history', { username: `Admin_${req.user.id}`, history: rows });
    });
});
...
app.post('/save-info', authenticateToken, (req, res) => {
    const info = req.body;
    db.run(`UPDATE users SET info = ? WHERE id = ?`, [JSON.stringify(info), req.user.id], (err) => {
...
app.post('/save-form-with-media', authenticateToken, upload.array('media', 10), (req, res) => {
    ...
    db.get(`SELECT * FROM lots WHERE qrCode = ?`, [qrCode], (err, row) => {
        ...
        if (!row && role !== 'farmer') return res.status(400).json({ message: 'Lô chưa được tạo' });

        const dataString = JSON.stringify(formData);
        if (row) {
            db.run(`UPDATE lots SET ${column} = ?, status = ? WHERE qrCode = ?`, 
                [dataString, statusMap[action] || row.status, qrCode]);
        } else {
            db.run(`INSERT INTO lots (qrCode, ${column}, status) VALUES (?, ?, ?)`, 
                [qrCode, dataString, statusMap[action]]);
        }

        db.run(`INSERT INTO history (qrCode, role, action, data, timestamp) VALUES (?, ?, ?, ?, ?)`, 
            [qrCode, role, action, dataString, timestamp]);

        const mediaUrls = mediaFiles ? mediaFiles.map(file => `/uploads/${file.filename}`) : [];
...
app.post('/admin/add-user', authenticateToken, (req, res) => {
    ...
    bcrypt.hash(password, 10, (err, hash) => {
        ...
        db.run(`INSERT INTO users (username, password, role, info) VALUES (?, ?, ?, ?)`, 
            [username, hash, role, JSON.stringify(info)], (err) => {
                if (err) return res.status(500).json({ message: 'Lỗi thêm người dùng' });
